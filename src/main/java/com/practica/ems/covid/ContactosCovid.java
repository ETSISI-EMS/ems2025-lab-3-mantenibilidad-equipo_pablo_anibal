...
public class ContactosCovid {
	...
	public void loadData(String data, boolean reset) throws EmsInvalidTypeException, EmsInvalidNumberOfDataException,
		...
		dividirComprobarData(data);

	}

	/**
	 Compruebo que cada línea
	 ...
	 */
	private void dividirComprobarData(String data) throws EmsInvalidTypeException, EmsInvalidNumberOfDataException, EmsDuplicatePersonException, EmsDuplicateLocationException {
	...
	private Persona crearPersona(String[] data) {
		...
		int i = 1;
		persona.setDocumento(data[i++]);
		persona.setNombre(data[i++]);
		persona.setApellidos(data[i++]);
		persona.setEmail(data[i++]);
		persona.setDireccion(data[i++]);
		persona.setCp(data[i++]);
		persona.setFechaNacimiento(parsearFecha(data[i++]));
		return persona;
	}
